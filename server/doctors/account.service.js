...
async function register(params, origin) {
    ...
    const isFirstAccount = (await db.Doctor.countDocuments({})) === 0;
...
async function update(id, params) {
    ...
    // validate
    if (doctor.email !== params.email && await db.Doctor.findOne({ email: params.email })) {
...
async function getAccount(id) {
    ...
    const doctor = await db.Account.findById(id);
...
function basicDetails(doctor) {
    const { id, title, firstName, lastName, email, role, created, updated, isVerified } = doctor;
